<template>
  <FormModal title="Edit Category" :is-show="isShow" @on-submit="editCategory">
    ...
    <div class="mt-5">
      ...
      <div class="mt-2">
        <FormUpload
          :files="modelValue?.image"
          @on-upload="handleImage"
...
function initModels() {
  models.name = props.modelValue?.name || ''
  models.slug = props.modelValue?.slug || ''
  models.image = props.modelValue?.image || ''
}
...
async function editCategory() {
  ...
  try {
    ...
    isError.value = false
    await categoryStore.editCategory(props.modelValue?.slug, models)
